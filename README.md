...
- **AI ラジオ DJ Bot** — 会話が途切れた際や「DJを呼ぶ」に応じて自動応答（LLMで自由生成。Gemini / OpenAI互換 API に対応。未設定時は何も投稿しない）。
...
- AI DJ（Gemini / OpenAI互換 API）、自動DJ局（人気曲/検索からランダム選曲）、専用局の自律運行（APScheduler）。
